...
class StringFieldTestCase(FieldTestCaseBase):
	def setUp(self):
		...
		self.field = StringField(self.keys[0], redis=self.redis, value_encoder=self.v_con)

	def test_set(self):
		self.redis.set(self.keys[0], self.v_con.encode(self.values[1]))

		result = self.field.set(self.values[2], False)

		self.assertFalse(result)
		self.assertEqual(self.v_con.decode(self.redis.get(self.keys[0])), self.values[1])


		result = self.field.set(self.values[3], True)

		self.assertTrue(result)
		self.assertEqual(self.v_con.decode(self.redis.get(self.keys[0])), self.values[3])
	...
	def test_get(self):
		...
		self.redis.set(self.keys[0], self.values_c[2])
